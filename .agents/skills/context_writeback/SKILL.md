...
1. 反映候補が次の4条件をすべて満たすか判定する。
   ...
   4. 保存先を1ファイルに特定できる。
2. 1つでも満たさない場合は反映しない。
3. 反映先を1つ選ぶ（存在する場合のみ適用）。
   - 最新スナップショット: `STATUS.md` 相当へ上書き。
   - 将来影響の決定: `DECISIONS.md` 相当へ短く追記。
   - 公開プロフィール文面: `profile_texts.md` 相当へ上書き。
   - オーナープロファイル（内部判断用）: `AGENTS.md` の `Owner Profile (Stable Context)` へ上書き。
4. 推測で補完せず、明示された内容だけを最小差分で反映する。
5. 同順位の情報源で矛盾があり未解決なら、`DECISIONS.md` 相当に `D-YYYYMM-xxx` 形式で暫定記録し、より新しい決定を優先して進める。
...
# Examples
- 「この判断を今後も使うので、`DECISIONS.md` に短く追記して」
- 「オーナープロファイルの技術ベースラインを、本人が明示した文言だけで更新して」
