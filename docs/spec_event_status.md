# spec_event_status — 公式の状態・日時訂正とLP表示
...
状態値、抑止・日時訂正の権限、統合キー、`summary`の出力契約、または公開前検証を変える場合は、この文書と実装・テストを同じ変更で更新する。`docs/spec_data.md` と `docs/spec_update_pipeline.md` は詳細を重複せず、この文書を参照する。
...
開催予定のまま、同一会場・出演者・titleの開催日、終了日、開始時刻だけを訂正する場合は `add_correction` を出力できる。中止・延期を捏造せず、旧source行と旧configを保持し、別event_idの公式訂正行を追加する。会場・出演者・title変更、既知時刻を未発表へ戻す変更、複数回の連続訂正、訂正と状態変更の混在は保留する。

Workが信頼済みbaseの2つのDBと現行LPを読み、同じ期間・review stateでLPを再現する。表示中の旧公演が一意で、その `supporting_sources` の実source_id/record_idと内容fingerprintが一致する場合だけ移行計画を作る。時刻不明の旧sourceが複数公演を支えている場合、新日時が別公演と衝突する場合、旧版や欠損がある場合は停止する。LPの `include_past=true` は90日窓も含むため、無期限の判定は `history_window_days=null` で行う。

内部の任意項目 `confirmed_events[].date_time_correction`（schema_version=1）は、base、提案・入力LP・入力source全体のfingerprint、旧公演key/fingerprint、変更前値、検証済み公式event、実origin、退役対象source_id/record_id/内容fingerprintを持つ。Chatの添付proofは受け付けず、Workの読み取り値から導出する。この項目を `labels_json` に保存する。Ticketjam起点では、旧候補はconflictのまま最新判断に `official_correction`（既存公式event形式）を添え、実差分に終了日も許容する。既存LP起点のkeyをTicketjam keyへ読み替えない。

既定のdiscovery生成は、厳密統合の前に公式訂正行と計画・最新reviewを照合し、指定した旧source行だけをメモリ上で外す。その後は従来の時刻分割・補助統合・状態抑止・source priorityを使う。DB行は削除しない。取得時刻だけの更新は許すが、旧行の内容・根拠の変化、未審査の別IDで同じ旧日時が再出現、同一旧行への重複した訂正は生成を停止し、再確認を要求する。既知の別時刻の公演は保持する。

訂正行は新日時が過去になっても読み取り、旧日時を退役させてから表示期間外の訂正行を外す。90日窓を越えた結果、まだ期間内の旧日時が復活してはならない。訂正を扱わない旧display/reviewed生成は停止する。訂正は中止・延期数へ加算しない。内部proofを公開LPへ追加しない。DB schema、Releaseの形、通常のsource priorityは変更しない。

移行・復帰: 本番適用は別承認。コード・対象config・Ticketjam reviewを一組として、最新baseの一時DBへ対象行だけを追加し、初回1件・再入力0件・旧行保持・別公演保持・経過後非復活を確認してLP/manifestを検証する。入力hashが変われば案を作り直す。適用失敗時は案を破棄し原本を保持する。適用済み状態でコードだけ戻したり、訂正行の無効化・削除だけを行うと旧日程が復活し得るため禁止する。復帰時は旧行と訂正行、config/review、コードを整合する検証済みsnapshotへ揃え、公式根拠とLP差分を再審査する。監査履歴は削除しない。一般の実体変更・連続訂正・自動適用は未対応。
...
処理順は `入力 -> 検証済み日時訂正の旧行退役 -> 厳密統合 -> 開始時刻分割 -> 補助統合 -> 延期・中止抑止 -> source priorityによる表示元選択` とする。アーティスト表記が異なる下位sourceも、補助統合で同一イベントになった後に抑止判定する。
