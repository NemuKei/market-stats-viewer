...
- D-20260206-006 | make_release_zip.py の include パターンを fnmatch 前提で `scripts/**` / `docs/**` に統一し、同梱漏れ（scripts/docs）が再発しない形にする | status: spec_pending | spec_link: START_HERE.md
...
- D-20260206-009 | openpyxl での推移表パースは `read_only=False` を採用し、セル参照型パースの性能劣化を回避する | status: spec_pending | spec_link: docs/spec_update_pipeline.md
