...
- 下書きの `origin`、`candidate_review_state`、`config_review_status` は公開fieldではない。既存configを渡したときは既存昇格処理と共通の同一origin・変更前config fingerprintの検証を通す。config未入力は `not_checked` と明記する。下書きの直接appendは行わず、下記の取込案を作成する。conflictによる旧開催予定行の掲載保留は維持する。純粋な状態変更の例外は `spec_event_status.md` を参照する。
