...
- D-20260512-003 | Codex automation によるイベント情報監査は初期運用では dry-run とし、監査レポート生成、低リスク修正案の作成、verify、PR作成までを許可するが、自動マージは行わない。automation は `data/event_signal_audit_report.json` の `automation_bucket`、`lp_impact`、`needs_review_reason` を読み、修正対象と人間確認対象を分ける | status: applied | spec_link: docs/event_signal_audit_automation.md
- D-20260512-004 | イベント情報監査の自動マージ可否は `docs/event_signal_audit_automation.md` の `Auto-merge Gate Checklist` で判定する。初期運用では `classification=auto_merge_candidate` でも `merge_action=do_not_merge` とし、実際の自動マージは行わない。`needs_review_reason`、配布DB/manifest/Release asset更新、DBスキーマ変更、ID変更、source優先順位変更、LP表示契約変更を含む変更は自動マージ禁止とする | status: applied | spec_link: docs/event_signal_audit_automation.md
