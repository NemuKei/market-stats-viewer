...
- D-20260413-001 | ICD更新では四半期 workbook だけでなく年間 workbook も同じ更新対象として扱う。年間 workbook の表題が `2025年（令和7年） 暦年【確報】` のように `暦年` を含む場合も、metadata は `period_label=YYYY年年間`、`period_key=YYYY`、`release_type=確報` として保存する | status: spec_done | spec_link: docs/spec_icd_update_pipeline.md
...
- D-20260502-003 | `ヤンマースタジアム長居` は旧 `www.nagai-park.jp` の WordPress 導線ではなく、新 `https://nagaipark.com/news/` の月次 `イベントカレンダー` PDF を会場公式に準じる公開スケジュールとして使う。PDF内で `施設・場所等` が `ヤンマースタジアム長居` の行だけを `events.sqlite` へ保存し、会場辞書は `is_enabled=1`, `ticketjam_watch=0`, `official_fetch_candidate=0` へ切り替える | status: applied | spec_link: docs/spec_update_pipeline.md
- D-20260512-001 | 会場公式以外のイベント情報は、まず取得漏れと正規化品質を監査する。監査対象は、取得頻度、取得入口、取得件数上限、parser抽出可否、同一イベント候補、会場名正規化、アーティスト名正規化、カテゴリ分類とする。Codex automation は本線取得処理そのものではなく、監査レポート生成、低リスク修正案の作成、verify、限定条件内の自動マージに使う。実装初期対象は `kstyle_music` とし、既存の `events.sqlite` / `event_signals.sqlite` のデータ契約は変更しない | status: planned | spec_link: docs/spec_update_pipeline.md
- D-20260512-002 | 市場統計とイベント情報はLP側でも利用するため、今後このリポジトリで `market_stats.sqlite`、`events.sqlite`、`event_signals.sqlite`、`manifest.json`、Release asset、関連workflow、表示用のカテゴリ・期間・集計・正規化ロジックに触れる実装では、LP側の表示件数、カテゴリ、集計値、同一イベントのまとまり、データ鮮度、配布manifestへの影響を確認する。影響がない場合も `lp_impact=none` 相当の理由を残す | status: applied | spec_link: AGENTS.md
- D-20260512-003 | Codex automation によるイベント情報監査は初期運用では dry-run とし、監査レポート生成、低リスク修正案の作成、verify、PR作成までを許可するが、自動マージは行わない。automation は `data/event_signal_audit_report.json` の `summary.automation_bucket_counts`、`summary.candidate_lp_impact_counts`、`summary.lp_impact` を確認し、候補配列内の各行が持つ `automation_bucket`、`lp_impact`、`needs_review_reason` と `needs_review` 一覧を使って、修正対象と人間確認対象を分ける | status: superseded_by_D-20260521-003 | spec_link: docs/event_signal_audit_automation.md
- D-20260512-004 | イベント情報監査の自動マージ可否は `docs/event_signal_audit_automation.md` の `Auto-merge Gate Checklist` で判定する。初期運用では `classification=auto_merge_candidate` でも `merge_action=do_not_merge` とし、実際の自動マージは行わない。`needs_review_reason`、配布DB/manifest/Release asset更新、DBスキーマ変更、ID変更、source優先順位変更、LP表示契約変更を含む変更は自動マージ禁止とする | status: superseded_by_D-20260521-003 | spec_link: docs/event_signal_audit_automation.md
- D-20260512-005 | 会場公式イベントのタイトル単体推論では、音楽イベントキーワードがない場合でも、辞書の canonical artist name がタイトル先頭に高信頼で一致し、かつ非音楽キーワードを含まない場合は artist/category 補完を採用する。alias だけの先頭一致は採用しない | status: applied | spec_link: docs/spec_update_pipeline.md
- D-20260521-001 | K-Style 取得漏れ監査で本文・日程・会場を確認済み、かつ現行parserで日本国内公演日程として抽出できる記事は、`kstyle_music` の `backfill_article_urls` に明示URLとして追加してよい。これは過去漏れ記事の限定補完であり、source優先順位、DBスキーマ、LP表示契約、Release asset更新条件は変更しない。`parser_gap`、海外公演、本文詳細未確認、または日本国内公演か判断できない候補は追加しない | status: applied | spec_link: docs/spec_update_pipeline.md
- D-20260521-002 | 次回以降のイベント情報監査automationは、K-Style `pr_candidate` を候補一覧で止めず、記事本文を確認して `import_ready`、`parser_patch_ready`、`out_of_scope`、`human_review_required` に再分類する。`import_ready` は `backfill_article_urls` 追加と `event_signals.sqlite` 限定取り込みまで実行し、`parser_patch_ready` は狭いparser形式対応とテスト追加後に限定取り込みしてよい。自動マージ、Release asset更新、manifest更新、source優先順位変更、DBスキーマ変更は引き続き禁止する | status: superseded_by_D-20260521-003 | spec_link: docs/event_signal_audit_automation.md
- D-20260521-003 | イベント情報監査automationは、低リスク候補を自動マージし、その直後に `Post-merge Audit` を実行する運用へ変更する。自動マージできるのは `docs/event_signal_audit_automation.md` の `Auto-merge Gate Checklist` をすべて満たす `auto_merge_candidate` だけとする。K-Style確認済み候補の限定取り込みは `source_id='kstyle_music'` の本文確認済みURLに対する限定追加または更新に限り自動マージ可とする。Release asset更新、manifest更新、source優先順位変更、DBスキーマ変更、ID変更、LP表示契約変更、parser全体再設計を含む変更は自動マージしない。マージ後監査で不一致が見つかった場合は、同じautomation内で隠して修正せず、後続の修正PRまたはrevert PRへ分ける | status: applied | spec_link: docs/event_signal_audit_automation.md
- D-20260521-004 | 複数の仕様判断、運用判断、automation 判断で再利用する判断原則の正本として `docs/context/INTENT.md` を導入する。人間確認を毎回増やすのではなく、automation が根拠、分類、変更案、verify 結果、LP 影響、post-merge audit 結果を出力し、人間確認は安全に判断できない候補へ限定する方向を一般原則として扱う。個別仕様は `docs/spec_*.md`、日付付き決定は `docs/context/DECISIONS.md`、次スレッドの再開地点は `docs/context/STATUS.md` に置く | status: applied | spec_link: docs/context/INTENT.md
- D-20260614-001 | Codex workflow は `main` 上の線形作業を既定とし、parallel worktree orchestration は利用者が `並列で進めて`、`別で進めて`、`裏で進めて` などを明示した場合だけ使う。親 thread が task 分割、shared / high-conflict files、integration、final verify、`main` への Git sync を所有し、子 thread は dedicated worktree / task branch の owned scope 内で local verify を報告する。子 thread は親または repo policy が明示しない限り `main` へ push / merge しない | status: applied | spec_link: AGENTS.md
