...
## Doing
- T-20260306-010: Ticketjamの新取得方針（会場辞書優先 + capacity>=1000 + 種別3区分）を仕様化する
...
- [ ] T-20260306-010: 取得方針を仕様化する（カテゴリ絞り解除 + 会場辞書一致 + `capacity >= 1000` + 種別3区分）
- [ ] T-20260306-011: `ticketjam_events` の取得時フィルタを「会場辞書基準」へ変更する（カテゴリslug必須を外す）
- [ ] T-20260306-012: 会場辞書一致かつ `capacity >= 1000` の採用ゲートを実装する
- [ ] T-20260306-013: Ticketjamデータの `event_category` を `コンサート / 野球 / その他` に分岐する（Ticketjamカテゴリ優先、不足時は既存ルールで補完）
- [ ] T-20260306-014: 二次流通ビューのGUIを会場公式ビュー寄りに揃える（種別フィルタ、列順、操作導線）
- [ ] T-20260306-015: `--ticketjam-bootstrap-full` 実行で再構築し、会場別件数と種別件数を検証する
- [ ] T-20260306-016: `DECISIONS` / `spec_update_pipeline` / `spec_app` を新方針へ同期する
...
Now:
- T-20260306-010, T-20260306-011

Next:
- T-20260306-012, T-20260306-013, T-20260306-014

After Next:
- T-20260306-015, T-20260306-016
