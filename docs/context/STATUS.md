...
- Docs governance profile: MSV は `AGENTS.md` を作業入口とし、`docs/context/PROJECT_CONTEXT.md` を optional upper premise layer として保持する。目的、source priority、LP影響、automation判断、仕様判断に触れる場合だけ `PROJECT_CONTEXT.md` の `Always Read Block` を読む。
...
- `docs/context/PROJECT_CONTEXT.md` を導入し、`docs/context/INTENT.md` は廃止メモへ縮退した。今後の判断原則、source-of-truth routing、LP掲載優先、完全自動化方針は `PROJECT_CONTEXT.md` の `Always Read Block` を入口にする。会場起点Web検知として repo 固有 Skill `.agents/skills/venue-web-discovery/SKILL.md`、設定 `data/venue_web_discovery_config.json`、source plugin `venue_web_discovery`、LP-ready 出力 `data/lp_events.json`、専用 workflow `update_signals_venue_web_discovery.yml` を追加した。本文抽出は `requests_bs4` を既定、`crawl4ai` を optional fallback provider とし、GitHub Actions では `VENUE_WEB_DISCOVERY_ENABLE_CRAWL4AI=true` のときだけ optional setup する。Bruno Mars と Stray Kids の既知例は公式/準公式根拠から `venue_web_discovery` signals と `lp_events.json` に反映する。LP表示source優先順位は `official_events > venue_web_discovery > starto_concert/kstyle_music > ticketjam_events`。配布assetに `lp_events.json` を追加するため `lp_impact=display_count_change,source_priority_change,manifest_asset_change`
- `docs/context/INTENT.md` を追加し、複数の仕様判断、運用判断、automation 判断で再利用する判断原則の正本を分離した。方針は、人間が毎回候補を読む運用ではなく、automation が根拠、分類、変更案、verify 結果、LP 影響、post-merge audit 結果を出力し、人間確認は安全に判断できない候補へ限定すること。`AGENTS.md` の Task Read、Source Priority、Local Extension も `INTENT` を参照するよう同期し、`docs/context/DECISIONS.md` に `D-20260521-004` を追加した。docs-only 変更であり、配布DB、manifest、Release asset、LP表示データは変更していないため `lp_impact=none`
