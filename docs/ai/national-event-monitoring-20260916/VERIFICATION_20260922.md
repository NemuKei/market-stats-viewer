...
台帳160→208（47追加＋最後のマツダスタジアム1）。開始時の元160行、別名168行はbytes単位で先頭一致。観測132のID・名前・従前evidenceは全件維持。既登録7件を追確認、将来施設6件は観測に残し、現行稼働会場の台帳には混ぜない。会場公式有効32、Web watch12、Ticketjam75（有効68）は維持。台帳で容量不明43。auditはID衝突・孤立・alias衝突なし。追加会場はすべてis_enabled=0/ticketjam_watch=0/official_fetch_candidate=1。

観測状態：運営主体確認・予定表待ち74、施設同定・予定表待ち50、将来施設の詳細待ち6、以前に一部予定表確認1、以前からの住所矛盾1、未審査0。**すべての予定表を読んだ意味ではない**。missing_fieldsは予定表範囲130、容量根拠48、運営主体38など。132観測の外側の全国施設探索も継続する。

- 大阪市公式の2026-04-01改称で「YANMAR HANASAKA STADIUM」は長居球技場。既登録の「ヤンマースタジアム長居」は別施設。新館扱いも同一ID化もしない。施設本文の20,000と24,665の表記差は保留。
- YKK AP ARENAは富山市総合体育館の改修・改称。9/22告知のオープニング本文は確認したが、開催日を示すPDFは未読。秋田新体育館は工事予定、静岡は契約締結、三河安城は2028年予定、京都・豊橋・（新）とどろきも別々の計画観測として保持。将来の開催日を生成しない。
- 藤枝の施設ページ内に13,000と10,057の収容表記が併存。マツダスタジアムは広島市の住所・名称・竣工時33,000を確認したが、**現行定員は空欄**。運営者や施設名の同定と、定員・公式予定表の確認は別に記録した。
- いわきの旧観測住所は公園事務所で、球技場本体の住所とは異なる。山形の公園名がBリーグで指す体育館は主催者別の再確認待ち。松山市の体育館は複合施設内のホールと区別。福井のアリーナは同名の野球場と区別。

### C・D：公式、発表元、補完のつなぎ方

今回の会場資料の検査記録は [SOURCE_CHECKS.json](SOURCE_CHECKS.json) の`round3_*`、公開ソース103件（本文取得91、取得失敗11、空本文1）。URL・観測UTC日時・取得方法・本文hash・失敗理由を保存し、raw本文と画像は含めない。公式トップのSNSへのリンク49件は「関連候補」で、アカウント投稿の取得・内容の確認は未実施。検索の見出しだけでイベントを生成していない。

12会場の予定表リンクは入口として控え、**全月・全件の閲覧完了とはしていない**。工事日程・体育館の開放枠・設営撤収を催事開催日/開演としない。前回確認したPDF6件10ページと日付矛盾もそのまま保持。8つの県別読取一覧は最大152,781 bytes、scope `9b7436e98e49cf319b22fcc5053d0e027e286e79e65e276ca64062fc1f240fdd`、すべてsnapshot_only。会場公式、全国発表元（アーティスト公式・ニュース・SNS）、チケジャム補完の3経路からの提案→Workの検証・取込案という既存受渡しを、このscopeと長崎のfixtureに接続。チケジャム単体やニュース/SNS単体では本番イベントを確定しない。

### E〜G：実行した検証と表示への影響

- `uv run --frozen python -m pytest -q`: **309 passed / 70 subtests passed、exit 0**。R1〜R3、日時訂正・中止延期、日次公平性・JST再試行・重複/排他の既存回帰を含む。今回は実装コードを変更していない。
- 既存dictionary-maintenance監査 exit 0（signals 1,408 / official 1,236）。新別名48行の正規名/別名76表記を実lookupで確認し、隣接スタジアム・体育館・複合施設など8組の誤統合なし。全国の追加別名候補を機械的に昇格していない。
- 7原本（3 DB、保存LP、Web取得設定、チケジャムページ・状態）は開始headとbytes一致。`data/manifest.json`は存在せず、一時ディレクトリでのみ生成。旧台帳行・旧別名行を保持。既存DBから再生成したLP1,144件のevent配列は、新規48会場を追加する前後で全行一致。今回の`lp_impact=none for this increment`。従前main保存1,142件との差は前回記録のIG+2、TOYOTA表示差を引き継ぐ。
- 既存実公演の長崎11/1野口五郎17:00を、新scopeで提案→判断→取込案→**コピーDB**へ接続。追加1件・再実行0、既存1,144行保持、試験LP1,145件とmanifest validator成功。試験したものを本番取込と呼ばない。

| 一時検証物 | bytes | SHA256 |
|---|---:|---|
| events.sqlite | 1,282,048 | `cb1cdfc0f6bd285f91fb154a31bf2e317aba78befe892348169d9eedb4dbaccf` |
| event_signals.sqlite | 4,194,304 | `840b7095cebcd94c1ce0bdbad20a58a2b64fdf9000f00f1eb19d83a842324bf7` |
| lp_events.json | 2,236,368 | `573d87d99c654fb271bbdb0d16c56fd0c2169c28ca019c18509a2d11334f74e4` |

入力SHA256: registry `efbcf70121f8d56c9540ec10aee8374257472c63fb3935d26c2ec7f04df432df`、aliases `38efec4a897507e3719451d1c6b71062c3293a11dbad19fe9fb6c77e3aac7db5`、census `9ffbc6b572fe842b01f366cfc5b50746ce4f036eed0d9d5eb23324b2add6d416`。一時manifestのcommitを最終PR headへ合わせた再検証結果はPR本文に記録する。これは公開assetのhashではない。

### F・H：まだ確認していない運用

定期Chat提出→Work Cloud自動起動→人の確認待ち→再実行、Cloud/Actions/旧端末の単一writer、実使用量・最大遅延は実測していない。既存タスクの変更・新設・手動起動はせず、PRは下書きのまま。権限・本番設定・原本DB・Release・利用側LP・旧端末を変更していない。`sync-needed`: 公式予定表の広域・全月/全件確認、全国発表元/SNS投稿の実読、定期Cloud受入と単一writer、公開前の最新版での再照合、切替承認と公開hash追跡。新規収集をオンにする前に各会場の受入条件を満たす。

## 15会場追加・予定表本文確認の検証（履歴）
