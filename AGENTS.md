...
`docs/ai/` にある handoff、plan、session snapshot など後続スレッドや ChatGPT / Codex 連携で再利用する資産は、基本的に commit / push 対象として扱う。ただし無条件に stage せず、秘密情報、個人情報、raw log 全文、巨大な一時出力、単なる作業ゴミ、端末固有メタ情報がないことを確認してから現在 task に関係するファイルだけを stage する。`.chatgpt/` は audit / log 系の repo 正本ではないツール実行履歴や一時メタ情報として扱い、repo 管理対象にしない。
